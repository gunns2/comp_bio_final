...
df = pd.read_csv('df_851.csv')
X = df.ix[:,['log_cov']].values
af = AffinityPropagation(preference = -10, max_iter = 4000, verbose=True, damping = 0.95, affinity = 'euclidean', convergence_iter = 400).fit(X)
cluster_centers_indices = af.cluster_centers_indices_
labels = af.labels_
df['cov_clusters'] = labels
clusters = df.ix[:,['cov_clusters']]
...
# 	af = AffinityPropagation(preference = -500, max_iter = 4000, verbose=True, damping = 0.95, affinity = 'euclidean', convergence_iter = 600).fit(X)
# 	cluster_centers_indices = af.cluster_centers_indices_
# 	labels = af.labels_
# 	n_iter = af.n_iter_
# 	no_clusters = len(cluster_centers_indices)
...
for cluster in df.cov_clusters.unique():
	print('cluster: ', cluster)
	df_sub = df.loc[df['cov_clusters'] == cluster]
	X = df_sub.ix[:,'GC_percent'] #looking at only tetra_nuc and GC content
	# X = df_sub.ix[:,['GC_percent']]
	X = X.values.reshape(-1,1)
	print('length X: ', len(X))
	af = AffinityPropagation(preference = -5, max_iter = 4000, verbose=True, damping = 0.95, affinity = 'euclidean', convergence_iter = 600).fit(X)
	cluster_centers_indices = af.cluster_centers_indices_
	labels = af.labels_
	n_iter = af.n_iter_
	no_clusters = len(cluster_centers_indices)
